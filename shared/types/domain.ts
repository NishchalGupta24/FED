...
export interface Sale extends Identity { saleId: string; customerId?: string; customerName?: string; customerPhone?: string; items: SaleItem[]; subtotal: number; discount: number; tax: number; total: number; paymentMethod: 'CASH' | 'UPI' | 'CREDIT'; paymentStatus: PaymentStatus; transactionId: string; deliveredAt?: string }
